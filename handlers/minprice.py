...
async def _match_offers_with_ai(game_name: str, lots_dict: dict, items_dict: dict) -> dict:
    ...
    prompt = f"""Ты — точный сопоставитель товаров.

ЗАДАЧА: Сопоставь каждый товар из «Товары в боте» с лотами из «Лоты FunPay». Один товар может соответствовать нескольким лотам (разные серверы/регионы).

═══ Лоты FunPay (offer_id: название) ═══
{lots_text}

═══ Товары в боте (твои строки ответа) ═══
{items_text}

═══ ПРАВИЛА СОПОСТАВЛЕНИЯ ═══

1. ТОЧНОЕ СОВПАДЕНИЕ: товар и лот — это ОДИН И ТОТ ЖЕ продукт.
   - Число ДОЛЖНО совпадать точно: "2000 BC" → только лоты с "2000 BC", НЕ "2500 BC", НЕ "1500 BC", НЕ "2000" без BC.
   - Название/валюта ДОЛЖНА совпадать: "2000 BC" → "2000 BC", а НЕ "2000 алмазов", НЕ "2000 кристаллов".

2. Слово «Бонусы» без числа = базовый лот «Бонусы [Цена за 1 шт.]» и подобные.

3. Наборы в кавычках: "Набор Олигарх" = лот с текстом "Олигарх".

4. Варианты с/без кэшбека получают ОДНИ И ТЕ ЖЕ offer_id:
   "100 бонусов (с кэшбеком)" и "100 бонусов (без кэшбека)" → одинаковые offer_id.

5. НЕ ДОПУСКАЕТСЯ привязка лотов с другим числом или другим названием!
   "2000 BC" ≠ "2500 BC" ≠ "1500 BC" ≠ "2000"

═══ ФОРМАТ ОТВЕТА ═══
Строго по одной строке на каждый товар. Используй символ → :
полное_название_товара → offer_id1, offer_id2

Пример:
100 бонусов (без кэшбека) → 47107842
2000 BC (без кэшбека) → 47125000
2000 BC (с кэшбеком) → 47125000
Набор "Олигарх" (без кэшбека) → 47125111

Если товар НЕ найден в лотах:
товар (без кэшбека) → не найдено

ВАЖНО:
- Отвечай ТОЛЬКО строками с →. Без пояснений.
- Каждый товар из списка выше ДОЛЖЕН быть в ответе.
- Не выдумывай offer_id — используй ТОЛЬКО из списка лотов."""
    ...
    try:
        ...
        # Fallback: если для товара нет привязки, ищем точное совпадение базового имени
        # (без суффикса кэшбека) среди уже найденных товаров
        for item_name in item_names:
            ...
            for key in result.keys():
                ...
                # Только точное совпадение базового имени (без кэшбек-суффикса)
                if base_name == key_base:
                    result[item_name] = result[key]
                    print(f"[AUTO-LINK AI FALLBACK] {item_name} → {result[key]} (взято из {key}, совпадение базового имени)")
                    break
...
async def cb_minprice(call: types.CallbackQuery, state: FSMContext):
    ...
    # --- Кнопка "нет кэшбека" при пошаговом добавлении ---
    elif data == "mp_bulk_no_cb":
        state_data = await state.get_data()
        photo_items = state_data.get("photo_items", [])
