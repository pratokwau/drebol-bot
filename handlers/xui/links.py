...
def build_instruction_text(vless_link: str, device_name: str = "", bot_username: str = "drebolwork_bot") -> str:
    ...
    return (
        ...
        "⚙️ <b>Настройка:</b>\n"
        "1. Откройте приложение → выберите язык Русский → на каждом следующем шаге нажимайте «Далее»\n"
        ...
        "3. Нажмите ⚙️ (шестерёнка) → Правила → Правила перенаправления → ✏️ карандаш → ⋯ три точки → Добавить\n"
        "4. В поле «Примечание» введите <code>RU</code>\n"
        "5. В <b>Rule Set (build-in)</b> добавьте:\n"
        "   <code>geosite:ru</code>\n"
        "   <code>geoip:ru</code>\n"
        "6. Нажмите ✓ (галочка сверху справа) → перетяните RU на самый верх списка личных правил\n"
        "7. Нажмите на RU → выберите Напрямую\n\n"

        "▶️ <b>Запуск:</b>\n"
        "8. На главном меню выберите режим Правила → в самом низу выберите Автовыбор → запустите VPN\n\n"
